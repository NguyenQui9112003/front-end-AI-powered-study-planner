...
export const TaskManagementPage = () => {
	...
	const fetchTasks = async () => {
		...
		try {
			let response = await fetch(`https://be-ai-study-planner.onrender.com/tasks?userName=${encodeURIComponent(
					user
				)}`,
				{
					method: 'GET',
			...
			if (response.ok) {
				...
				response = await fetch(`https://be-ai-study-planner.onrender.com/tasks?userName=${encodeURIComponent(
					user
				)}`,
					{
						method: 'GET',
	...
	const deleteTask = async (taskName: string) => {
		...
		try {
			...
			if (response.ok) {
				...
				accessToken = await getRefreshToken(refreshToken);
				response = await fetch('https://be-ai-study-planner.onrender.com/tasks/delete', {
					method: 'POST',
					headers: {
						Authorization: `Bearer ${accessToken}`,
						'Content-type': 'application/json',
